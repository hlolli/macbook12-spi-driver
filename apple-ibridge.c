...
static const struct hid_device_id appleib_sub_hid_ids[] = {
...
struct appleib_device {
	...
	struct hid_device_id		ib_dev_ids[ARRAY_SIZE(appleib_sub_hid_ids)];
};
...
static int appleib_probe(struct acpi_device *acpi)
{
	...
	memcpy(ib_dev->ib_dev_ids, appleib_sub_hid_ids,
